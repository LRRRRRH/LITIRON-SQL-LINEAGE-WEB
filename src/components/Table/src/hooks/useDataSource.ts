...
export function useDataSource(
  ...
  async function fetch(opt?) {
    try {
      ...
      let pageParams = {};
      const { pageNumber = 1, pageSize = 10 } = unref(getPaginationInfo) as PaginationProps;

      if ((isBoolean(pagination) && !pagination) || isBoolean(getPaginationInfo)) {
        pageParams = {};
      } else {
        pageParams[pageField] = (opt && opt[pageField]) || pageNumber;
      ...
      if (resultTotal) {
        ...
        if (pageNumber > currentTotalPage) {
          setPagination({
            pageNumber: currentTotalPage,
            itemCount: total,
          });
